...
const Create = () => {
  ...
  const openPicker = async (selectType) => {
    ...
    if (!result.canceled)
      if (selectType === 'image') {
        ...
        console.log("form image: "+ form.thumbnail)
      }
    if (selectType === 'video') {
      setForm({ ...form, video: result.assets[0] })
      console.log("form video: "+form.video)          // dneeme
    } //else {
    //   setTimeout(() => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      //console.log("user ıd: "+userId)             // eklendi

      Alert.alert('Succes', 'Post uploaded successfully')
      router.push('/home')
    } catch (error) {
      Alert.alert('Error'+ error.message)             // error mesaj eklendi.
